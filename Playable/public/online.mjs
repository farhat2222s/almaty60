...
const PERIOD=2000,MAX_BACKOFF=60000;

/** Validate and normalise a backend address typed by the player. Returns '' for an empty value, throws on bad input. */
export function normalizeBackendUrl(value,pageProtocol=globalThis.location?.protocol){
  const v=(value||'').trim();if(!v)return '';
  let u;try{u=new URL(v.includes('://')?v:'https://'+v)}catch{throw new Error('Некорректный адрес сервера')}
  if(!['http:','https:'].includes(u.protocol))throw new Error('Адрес должен начинаться с http:// или https://');
  const local=['localhost','127.0.0.1','[::1]'].includes(u.hostname);
  if(pageProtocol==='https:'&&u.protocol==='http:'&&!local)throw new Error('Со страницы https сервер тоже должен быть https');
  return u.origin+u.pathname.replace(/\/$/,'');
}

export function createOnline({getPosition,onPlayers=()=>{},onStatus=()=>{}}={}){
  let config={url:''},token=read(KEY_TOKEN),name=read(KEY_NAME),timer=null,busy=false,failures=0,status={state:'off',online:0,error:''};
  ...
  async function call(path,body,{auth=true}={}){
    ...
    if(!res.ok){const err=new Error(data.message||data.error||`HTTP ${res.status}`);err.status=res.status;err.retryAfter=Number(res.headers?.get?.('Retry-After'))||0;throw err;}
    return data;
  }
  ...
  // One request at a time, rescheduled after each answer: a dead or slow server gets exponential backoff (up to 60 s)
  // and a 429 honours Retry-After instead of hammering the rate limiter.
  async function tick(){
    if(!token||busy)return;
    let delay=PERIOD;const p=getPosition();
    if(p){
      busy=true;
      try{const d=await call('/api/presence',{x:p.x,z:p.z,heading:p.heading||0,driving:!!p.driving});if(!token)return;failures=0;onPlayers(d.players||[]);set({state:'on',online:d.online||1,error:''});}
      catch(e){failures++;delay=e.status===429?Math.max(5000,(e.retryAfter||60)*1000):Math.min(MAX_BACKOFF,PERIOD*2**Math.min(failures,5));if(failures>=3){onPlayers([]);set({state:token?'error':'off',error:e.message});}}
      finally{busy=false;}
    }
    if(timer!==null&&token)schedule(delay);
  }
  function schedule(ms){if(timer)clearTimeout(timer);timer=setTimeout(tick,ms);}
  function start(){stop();if(!token||!url())return;failures=0;set({state:'connecting',error:''});timer=0;tick();}
  function stop(){if(timer)clearTimeout(timer);timer=null;}
  function setUrl(value){const v=normalizeBackendUrl(value);write(KEY_URL,v);if(token)start();}
